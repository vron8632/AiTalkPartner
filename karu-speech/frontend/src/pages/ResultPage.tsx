...
import { ArrowLeft, Sparkles, Lightbulb, Target, Loader2 } from 'lucide-react'
...
export function ResultPage() {
  ...
  useEffect(() => {
    if (state) { setData(state); return }
    if (!recordId) return
    api.get(`/practice/records/${recordId}/`).then(r => {
      const d = r.data
      setData(d.scores ? {
        scores: d.scores,
  ...
  return (
    <div>
      ...
      {data.transcript && (
        <div className="bg-background rounded-xl border border-border p-4 mb-4 text-sm text-text-muted whitespace-pre-wrap">
          <div className="font-medium text-text mb-1">📝 你的演讲内容</div>
          {data.transcript}
        </div>
      )}
            ...
            return (
              <div key={key}>
                ...
                <div className="w-full bg-background rounded-full h-2.5 overflow-hidden">
                  <div className={`h-full rounded-full transition-all duration-700 ${scoreColors[key] || 'bg-primary'}`}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        {data.strengths?.length > 0 && (
          <div className="bg-surface rounded-xl border border-border p-5 shadow-sm">
            <div className="flex items-center gap-2 mb-3"><Lightbulb size={18} className="text-green-500" /><h3 className="font-bold text-text text-sm">优点</h3></div>
            <ul className="space-y-2">
              {data.strengths.map((s: string, i: number) => (
                <li key={i} className="flex items-start gap-2 text-sm text-text"><span className="text-green-500 mt-0.5 shrink-0">✓</span>{s}</li>
              ))}
            </ul>
          </div>
        )}
        {data.improvements?.length > 0 && (
          <div className="bg-surface rounded-xl border border-border p-5 shadow-sm">
            <div className="flex items-center gap-2 mb-3"><Target size={18} className="text-orange-500" /><h3 className="font-bold text-text text-sm">改进点</h3></div>
            <ul className="space-y-2">
              {data.improvements.map((s: string, i: number) => (
                <li key={i} className="flex items-start gap-2 text-sm text-text"><span className="text-orange-500 mt-0.5 shrink-0">!</span>{s}</li>
              ))}
            </ul>
          </div>
        )}
      ...
      <Link to="/" className="block w-full py-3 bg-primary text-white rounded-xl text-sm font-medium hover:bg-primary-light transition-colors text-center shadow-sm">继续练习</Link>
    </div>
  )
}
